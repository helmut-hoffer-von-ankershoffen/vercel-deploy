...
const run = async () => {
	...
	if (IS_FORK === true && DEPLOY_PR_FROM_FORK === false) {
		...
		const body = `
			...
			**@${ USER }** To allow this behaviour set \`DEPLOY_PR_FROM_FORK\` to true ([more info](https://github.com/helmut-hoffer-von-ankershoffen/vercel-deploy#deploying-a-pr-made-from-a-fork-or-dependabot)).
		`
